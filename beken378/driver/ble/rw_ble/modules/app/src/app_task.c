...
static int gapm_cmp_evt_handler(kernel_msg_id_t const msgid,
                                ...
                                kernel_task_id_t const src_id)
{
    printf("gapm_cmp_evt_handler param->operation = 0x%x, param->status = 0x%x \r\n", param->operation, param->status);
...
#if 0
				if( param->peer_addr_type == 1 && app_sec_env.bonded)//增加其它限制条件  //for test resolv peer addr :sean 2017.10.30
				{
...
#else
        if(ais_bt_init_info.on_connected != NULL)
		{
                                ...
                                kernel_task_id_t const src_id)
{
	...
	switch(param->operation)
	{
		...
		//	UART_PRINTF("GAPC UPDATE PARAMS STATE: 0x%x\r\n", param->status);
    	} break;
    	case (GAPC_BOND): // 0xa
		{
			if (param->status != GAP_ERR_NO_ERROR)
    	...
    	case (GAPC_ENCRYPT): // 0xb
		{
			...
			{
				//UART_PRINTF("GAPC_ENCRYPT Start ok !\r\n");
                                      ...
                                      kernel_task_id_t const src_id)
{
	...
	bk_printf("disconnect link reason = 0x%x\r\n",param->reason);
#if UPDATE_CONNENCT_PARAM
...
#else
	if(ais_bt_init_info.on_disconnected!= NULL)
    {
...
static int gapm_adv_report_ind_handler(kernel_msg_id_t const msgid,
                                     struct adv_report const *ind,
                                     kernel_task_id_t const dest_id,
                                     kernel_task_id_t const src_id)
{
    uint8_t find = 0;

    find = appm_adv_data_decode(ind->data_len, ind->data);
